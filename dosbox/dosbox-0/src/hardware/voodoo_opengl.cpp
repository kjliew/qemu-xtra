...
static void drawstr(const char *str, const int colors)
{
    ...
    glPushMatrix();
    glLoadIdentity();
    glOrtho(0, scale_w, 0, scale_h, -1, 1);
...
static void annotate_last(void)
{
    if (fxstats.base)
...
static void annotate_stat(void)
{
...
void voodoo_ogl_draw_pixel(int x, int y, bool has_rgb, bool has_alpha, int r, int g, int b, int a) {
	...
	VOGL_SetAlphaMode(0, 0,0,0,0);
        ...
        if (scale_width) {
            ...
            glVertex2f((1.f*x)+0.5f,(1.f*y)+0.5f);
            glVertex2f((1.f*x)+1.5f,(1.f*y)+0.5f);
            glVertex2f((1.f*x)+1.5f,(1.f*y)+1.5f);
            glVertex2f((1.f*x)+0.5f,(1.f*y)+1.5f);
        }
        else {
...
//	if (FBZMODE_Y_ORIGIN(v->reg[fbzMode].u))
	...
	if (scissors_needed) {
                unsigned int is = (((scale_width)? scale_width:v->fbi.width) << 16) / v->fbi.width;
		glEnable(GL_SCISSOR_TEST);
		glScissor((sx*is)>>16,(sy*is)>>16,((ex-sx)*is)>>16,((ey-sy)*is)>>16);
	}
...
UINT32 voodoo_ogl_read_pixel(int x, int y) {
	...
	if ((x < 0) || (y < 0) || (x >= (INT32)v->fbi.width) || (y >= (INT32)v->fbi.height)) return 0xffffffff;
        ...
        unsigned int win_w, win_h, is;
        if (scale_width) {
            float win_r = (1.f * v->fbi.height) / v->fbi.width;
            win_w = scale_width;
            win_h = win_w * win_r;
            is = (scale_width << 16) / v->fbi.width;
            y = (y*is) >> 16;
        }
        else {
            is = (v->fbi.width << 16) / v->fbi.width;
            win_w = v->fbi.width;
            win_h = v->fbi.height;
        }
	...
	switch (LFBMODE_READ_BUFFER_SELECT(v->reg[lfbMode].u)) {
		case 0:			/* front buffer */
			...
			if ((cached_line_front_y != y) || (x+1 >= cached_line_front_width)) {
				if (cached_line_front_length<(INT32)win_w) {
					if (cached_line_front_data!=NULL) free(cached_line_front_data);
					size_t span_length=((win_w+64)&(~15));
					cached_line_front_data=(UINT32*)malloc(sizeof(UINT32)*span_length);
					cached_line_front_length=(INT32)span_length;
				}
				glReadPixels(0,win_h-y,win_w,1,mode,GL_UNSIGNED_BYTE,cached_line_front_data);
				cached_line_front_y=y;
				cached_line_front_width = (INT32)win_w;
			}
			data[0]=cached_line_front_data[(x*is)>>16];
			data[1]=cached_line_front_data[((x+1)*is)>>16];
			break;
		case 1:			/* back buffer */
			VOGL_SetReadMode(false);
			if ((cached_line_back_y != y) || (x+1 >= cached_line_back_width)) {
				if (cached_line_back_length<(INT32)win_w) {
				...
				glReadPixels(0,win_h-y,win_w,1,mode,GL_UNSIGNED_BYTE,cached_line_back_data);
				cached_line_back_y=y;
				cached_line_back_width = (INT32)win_w;
			}
			data[0]=cached_line_back_data[(x*is)>>16];
			data[1]=cached_line_back_data[((x+1)*is)>>16];
			break;
		case 2:			/* aux buffer */
			mode=GL_DEPTH_COMPONENT;
			VOGL_SetReadMode(false);
			glReadPixels((x*is)>>16,win_h-y,2,1,mode,GL_UNSIGNED_INT,&data);
...
void voodoo_ogl_reset_videomode(void) {
	...
	GFX_TearDown();
        ...
        float win_r, r = (1.f * v->fbi.height) /v->fbi.width;
        Bitu win_w = GFX_ScaleWidth(win_r), win_h;
        if (v->fbi.width > (win_w / win_r)) {
            win_w = v->fbi.width;
            win_h = v->fbi.height;
        }
        else {
        ...
        scale_width = win_w * win_r;

	bool full_sdl_restart = !GFX_IsOpenGL();
	...
	if (depth_csize == 24) {
        ...
        if (VOODOO_FpsLimit())
            LOG_MSG("VOODOO: Frame rate limit [ %d FPS ]", VOODOO_FpsLimit());
	...
	for (int hct=0; hct<RASTER_HASH_SIZE; hct++) {
		...
		for (; info; info = info->next) {
			if (info->shader_ready) {
				...
				if (info->so_shader_program) {
					if (info->so_vertex_shader) glDetachObjectARB(info->so_shader_program, info->so_vertex_shader);
					if (info->so_fragment_shader) glDetachObjectARB(info->so_shader_program, info->so_fragment_shader);
					if (info->so_vertex_shader) glDeleteObjectARB(info->so_vertex_shader);
					if (info->so_fragment_shader) glDeleteObjectARB(info->so_fragment_shader);
